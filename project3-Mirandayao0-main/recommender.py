...
class Recommender(object):
    ...
    def train_user_euclidean(self, data_set, userId):
        ...
        myDict = {}
        iCol = 0
        for column in data_set:  # skip first column
            iCol = iCol + 1
            if iCol > 1:
                ...
                myDict[column] = euclidDistance ** 0.5
    ...
    def predict_user_existing_ratings_top_k(self, data_set, sim_weights, userId, k):
        sim_weights_sorted = dict(
            sorted(sim_weights.items(), key=lambda item: item[1], reverse=True))  # need to kick off 1st one, itself
        predResult = []
